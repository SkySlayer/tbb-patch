...
template<typename Range, typename Body>
void parallel_reduce( const Range& range, Body* body ) {
  __TBB_STATIC_ASSERT(internal::check_the_assert_message<Body>::value, __TBB_ASSERT_MSG_PARALLEL_REDUECE_BODY_SHOULD_BE_REFERENCE_INSTEAD_OF_POINTER);
}

template<typename Range, typename Body, typename Af>
void parallel_reduce( const Range& range, Body* body, const Af & partitioner ) {
  __TBB_STATIC_ASSERT(internal::check_the_assert_message<Body>::value, __TBB_ASSERT_MSG_PARALLEL_REDUECE_BODY_SHOULD_BE_REFERENCE_INSTEAD_OF_POINTER);
}
